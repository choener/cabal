...
data LinkedComponent
    = LinkedComponent {
        lc_uid :: UnitId, -- hashed!
        lc_pkgid :: PackageId,
        lc_cid :: ComponentId,
        lc_insts :: [(ModuleName, Module)],
...
instance IsNode LinkedComponent where
    ...
    nodeKey = lc_uid
    nodeNeighbors n =
        if Set.null (unitIdFreeHoles (lc_uid n))
...
toLinkedComponent verbosity this_pid pkg_map ConfiguredComponent {
    cc_cid = this_cid,
    cc_pkgid = pkgid,
    ...
    let
        ...
        unlinked_includes :: [((UnitId, ModuleShape), PackageId, IncludeRenaming)]
        unlinked_includes = [ (lookupUid cid, pid, rns)
                            | (cid, pid, rns) <- cid_includes ]

        lookupUid :: ComponentId -> (UnitId, ModuleShape)
    ...
    (linked_shape0   :: ModuleScope,
     linked_deps     :: [(UnitId, PackageId)],
     linked_includes :: [(UnitId, ModuleRenaming)]) <- orErr $ runUnifyM verbosity $ do
    ...
    let reqs = modScopeRequires linked_shape0
        ...
        insts = [ (req, ModuleVar req)
                | req <- Set.toList reqs ]
        this_uid = if Set.null reqs
                    then SimpleUnitId this_cid
                    else UnitId this_cid . Map.fromList $ insts
...
toLinkedComponents
    ...
    -> [ConfiguredComponent]
    -> LogProgress [LinkedComponent]
toLinkedComponents verbosity this_pid lc_map0 comps
   = fmap snd (mapAccumM go lc_map0 comps)
 where
  go :: Map ComponentId (UnitId, ModuleShape)
     -> ConfiguredComponent
     -> LogProgress (Map ComponentId (UnitId, ModuleShape), LinkedComponent)
  go lc_map cc = do
    lc <- toLinkedComponent verbosity this_pid lc_map cc
