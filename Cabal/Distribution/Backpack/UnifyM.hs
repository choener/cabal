...
runUnifyM :: Verbosity -> (forall s. UnifyM s a) -> Either String a
runUnifyM verbosity m
    = runST $ do i    <- newSTRef 0
                 hmap <- newSTRef Map.empty
                 unUnifyM m (UnifEnv i hmap verbosity Nothing)
...
convertUnitId' _ uid@(HashedUnitId _ _) = error $ "convertUnidId: illegal HashedUnitId " ++ display uid
...
-- unification on it.
convertInclude :: ((UnitId, ModuleShape), PackageId, IncludeRenaming)
...
convertInclude ((uid, ModuleShape provs reqs), pid, incl@(IncludeRenaming prov_rns req_rns)) = do
    ...
    --
    --      A -> X      ==>     A -> <X>
    let req_subst = fmap ModuleVar req_rename
