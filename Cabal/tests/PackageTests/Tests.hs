...
tests config = do
  ...
  tcs "ReexportedModules" "p" . whenGhcVersion (>= Version [7,9] []) $ do
      withPackageDb $ do
  ...
  tcs "Ambiguity" "reexport" . whenGhcVersion (>= Version [7,9] []) $ do
    withPackageDb $ do
        withPackage "p" $ cabal_install []
        withPackage "q" $ cabal_install []
  ...
  tcs "Backpack/Includes2" "internal" . whenGhcVersion (>= Version [8,1] []) $ do
    withPackageDb $ do
  ...
  tcs "Backpack/Includes2" "internal-fail" . whenGhcVersion (>= Version [8,1] []) $ do
    withPackageDb $ do
      r <- shouldFail $ cabal' "configure" ["--cabal-file", "fail.cabal"]
      assertOutputContains "mysql" r

  tcs "Backpack/Includes2" "external" . whenGhcVersion (>= Version [8,1] []) $ do
    withPackageDb $ do
  ...
  tcs "Backpack/Includes2" "per-component" . whenGhcVersion (>= Version [8,1] []) $ do
    withPackageDb $ do
  ...
  tcs "Backpack/Includes3" "internal" . whenGhcVersion (>= Version [8,1] []) $ do
    withPackageDb $ do
      cabal_install []
      -- TODO: refactorize
      pkg_dir <- packageDir
  ...
  tcs "Backpack/Includes3" "external-fail" . whenGhcVersion (>= Version [8,1] []) $ do
    withPackageDb $ do
  ...
  tcs "Backpack/Includes3" "external-ok" . whenGhcVersion (>= Version [8,1] []) $ do
    withPackageDb $ do
  ...
  tcs "Backpack/Includes3" "external-explicit" . whenGhcVersion (>= Version [8,1] []) $ do
    withPackageDb $ do
  ...
  tc "Backpack/Includes5" . whenGhcVersion (>= Version [8,1] []) $ do
      cabal "configure" []
      r <- shouldFail $ cabal' "build" []
  ...
  tc "Backpack/Reexport1" . whenGhcVersion (>= Version [8,1] []) $ do
    withPackageDb $ do
